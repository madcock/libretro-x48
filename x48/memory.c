...
{
    // fprintf(stderr, "write_nibble_gx %d %d\n", addr, val);
...
{
    // fprintf(stderr, "%s(%d) [%s]: Read nibbles sx %04X\n", __func__, __LINE__, __FILE__,addr);
...
{
    // fprintf(stderr, "%s(%d) [%s]: Read nibbles gx %04X\n", __func__, __LINE__, __FILE__,addr);
...
long read_nibbles(long addr, int len)
{
    ...
    // fprintf(stderr, "%s(%d) [%s]: Read nibbles %d\n", __func__, __LINE__, __FILE__,len);

    addr += len;
    while (len-- > 0)
        val = (val << 4) | read_nibble(--addr);

    // fprintf(stderr, "%s(%d) [%s]: End Read nibbles %d\n", __func__, __LINE__, __FILE__,len);
    return val;
}
