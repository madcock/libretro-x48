...
void init_nibble_maps(void)
{
  ...
  for (i = 0; i < 16; i++) {
    // nibble_maps[i] = XCreateBitmapFromData(dpy, disp.win, (char *)nibbles[i], 8, 2);
  }
#ifdef HAVE_XSHM
  if (shm_flag) {
...
void update_display(void)
{
  ...
  // display.on = 1; // REDBUG HACK TO REMOVE

  fprintf(stderr, "%s(%d) [%s]: update_display (%d)\n", __func__, __LINE__, __FILE__, display.on);

  if (!disp.mapped)
  {
    fprintf(stderr, "%s(%d) [%s]: disp not mapped\n", __func__, __LINE__, __FILE__);
  ...
  if (display.on) {
    ...
    // fprintf(stderr, "%s(%d) [%s]: update_display %d x %d (%d, %d)\n", __func__, __LINE__, __FILE__, NIBBLES_PER_ROW, display.lines, display.disp_start, shm_flag);
    ...
    if (shm_flag) {
      ...
      line_pad = disp.disp_image->bytes_per_line - line_length;

      // fprintf(stderr, "%s(%d) [%s]: line_pad %d\n", __func__, __LINE__, __FILE__, line_pad);
    ...
    if (i < DISP_ROWS) {
      ...
      if (shm_flag) {
        data_addr = 0;
        line_pad = disp.menu_image->bytes_per_line - NIBBLES_PER_ROW;
        for (; i < DISP_ROWS; i++) {
    ...
    if (shm_flag) {
      ...
      fprintf(stderr, "%s(%d) [%s]:reset display\n", __func__, __LINE__, __FILE__);


      memset(disp.disp_image->data, 0, (size_t)(disp.disp_image->bytes_per_line * disp.disp_image->height));
      memset(disp.menu_image->data, 0, (size_t)(disp.menu_image->bytes_per_line * disp.menu_image->height));
  ...
  // refresh_display(); // REDBUG HACK TO REMOVE
...
void disp_draw_nibble(word_20 addr, word_4 val)
{
  ...
  if (display.nibs_per_line != 0) {
    ...
    if (y < 0 || y > 63)
      return;
    if (shm_flag) {
      shm_addr = (y * disp.disp_image->bytes_per_line) + x;
...
void menu_draw_nibble(word_20 addr, word_4 val)
{
  ...
  if (shm_flag) {
    shm_addr = (offset / NIBBLES_PER_ROW) * disp.menu_image->bytes_per_line + (offset % NIBBLES_PER_ROW);
