...
char enter_debugger = 0;
#define TRAP_INSTRUCTION    1
...
int decode_group_80(void)
{
    ...
    // fprintf(stderr, "%s(%d) [%s]: decode_group_80 op3 %04X\n", __func__, __LINE__, __FILE__,op3);
...
int decode_8_thru_f(int op1)
{
    ...
    // fprintf(stderr, "%s(%d) [%s]: decode_8_thru_f op2 %04X\n", __func__, __LINE__, __FILE__,op2);
...
inline int step_instruction(void)
{
    ...
    // fprintf(stderr, "%s(%d) [%s]: Return nibbles %04X\n", __func__, __LINE__, __FILE__,op0);

    switch (op0) {
        case 0:
            op1 = read_nibble(saturn.PC + 1);

            // fprintf(stderr, "%s(%d) [%s]: Return nibbles op1 %04X\n", __func__, __LINE__, __FILE__,op1);
        ...
        case 7:
            ...
            break;
        default:
            // fprintf(stderr, "%s(%d) [%s]: decode_8_thru_f %04X\n", __func__, __LINE__, __FILE__,op0);
...
void emulate_frame(void)
{
    ...
    do {
        int i;
        // fprintf(stderr, "%s(%d) [%s]: Before step_instruction\n", __func__, __LINE__, __FILE__);

        step_instruction();

        // fprintf(stderr, "%s(%d) [%s]: After step_instruction\n", __func__, __LINE__, __FILE__);
...
//  schedule_event = 0;
        ...
        if (schedule_event-- <= 0) {
            // fprintf(stderr, "%s(%d) [%s]: Before schedule: %d %d\n", __func__, __LINE__, __FILE__ ,max, disp.display_update);
